...
class LiveViewer:
    ...
    def __init__(
        ...
        loop_bind_host = "127.0.0.1" if host in ("0.0.0.0", "::") else host
        try:
        ...
        self._thread = threading.Thread(
            target=self._uvicorn_server.run, name="simview-live-server", daemon=True
        )
        self._thread.start()

        # Block __init__ until the socket is actually bound, so a caller's
        # very first push_state (or a browser opened right after construction)
        # never races server startup.
        deadline = time.monotonic() + _START_TIMEOUT
        while not self._uvicorn_server.started:
            if not self._thread.is_alive():
                raise RuntimeError("SimView live server thread died during startup.")
            if time.monotonic() > deadline:
                raise TimeoutError(
                    f"SimView live server did not start within {_START_TIMEOUT}s."
                )
            time.sleep(_START_POLL_INTERVAL)

        logger.info(
            "SimView live server running on http://%s:%s", loop_bind_host, self.port
        )
        if open_browser:
            import webbrowser

            webbrowser.open(f"http://{loop_bind_host}:{self.port}")
